...
def validate_configuration():
    ...
    if not CLOUDINARY_CLOUD_NAME:
        errors_list.append(
            "❌ CLOUDINARY_CLOUD_NAME not set in environment variables")
    if not CLOUDINARY_API_KEY:
        errors_list.append(
            "❌ CLOUDINARY_API_KEY not set in environment variables")
    if not CLOUDINARY_API_SECRET:
        errors_list.append(
            "❌ CLOUDINARY_API_SECRET not set in environment variables")

    if not METADATA_FILE:
        errors_list.append(
            f"❌ Metadata file not found: bulk_import/metadata.csv or bulk_import/metadata.xlsx")
...
def read_metadata():
    ...
    try:
        if not METADATA_FILE:
            logger.error(
                "❌ Metadata file not found (checked for metadata.csv and metadata.xlsx)")
            return None
        ...
        if missing_columns:
            logger.error(
                f"❌ Missing columns in metadata file: {missing_columns}")
            return None
        ...
        if 'author' not in df.columns:
            df['author'] = 'UABC Team'
            logger.info(
                "ℹ️  No 'author' column found, using default: 'UABC Team'")
...
def create_insight_document(row, pdf_url):
    ...
    try:
        ...
        publish_date = pd.to_datetime(
            row['publishDate'], format='%d/%m/%Y').to_pydatetime()
    except:
        try:
...
def bulk_import_insights():
    ...
    for idx, row in tqdm(df.iterrows(), total=total, desc="Progress", unit="PDF"):
        ...
        try:
            ...
            if not pdf_path:
                logger.warning(
                    f"⚠️  PDF file not found: {filename} (Category: {category})")
                skipped += 1
                continue
