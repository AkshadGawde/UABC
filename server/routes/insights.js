...
const insightValidation = [
  ...
  body('category')
    .isIn(['Technology', 'Business', 'Innovation', 'Industry', 'Research', 'Analysis'])
...
// @access  Private (Editor+)
router.post('/', authenticateToken, requireEditor, insightValidation, async (req, res) => {
  try {
...
// @desc    Update insight
// @access  Private (Editor+)
router.put('/:id', authenticateToken, requireEditor, insightValidation, async (req, res) => {
  try {
    // Check validation errors
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
